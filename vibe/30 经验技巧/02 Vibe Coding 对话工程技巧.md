...
在上一篇文章里，我们讲了 Vibe Coding 的 5 个核心心法。今天我们要深入探讨其中最关键的一个技能，就是如何和 AI 进行高效对话。
...
很多朋友把和 AI 交流称为 “写提示词”、或者 “提示词工程”，我之前也是这样的。但如今，随着 AI 对模糊指令的理解能力越来越强，我觉得这个说法其实不够全面，因为它暗示了一种单向的、一次性的交流方式，好像你只要写出一个完美的提示词，AI 就会给你一个完美的答案。
...
前面讲的都是日常对话中的实用技巧，但如果你想更深入地理解提示词工程，一个很好的学习方式是去研究知名 AI 编程工具和大模型本身的提示词是怎么设计的。毕竟这些提示词是行业顶尖团队花大量时间打磨出来的，里面藏着很多经过验证的最佳实践。
...
### 学习 Claude Fable 5 的提示词设计

2026 年 6 月，Claude Fable 5 的完整系统提示词被泄露，整整 12 万字符、约 3 万 tokens。虽然这是给 AI 产品设计的系统提示词，但里面的设计思路对我们日常写 CLAUDE.md、AGENTS.md 这类规则文件同样适用。

研究这份提示词后，我发现了几个非常值得学习的设计思路：

**1）把重心放在「AI 能做什么」而不是「AI 是谁」。** 这份提示词超过一半的 token 花在了工具定义和搜索规则上，而「你是 Claude 由 Anthropic 创建」这句身份声明出现在接近最后的位置。所以写提示词的时候，与其花大量篇幅写人设，不如把精力放在工具使用规则和具体行为规范上。

**2）把踩过的坑直接写成规则。** 提示词里有很多看似莫名其妙的硬编码规则，比如「搜索 latest iPhone 2025 在 2026 年会返回过期结果，应该搜 latest iPhone 2026」。每一条背后都是用户真实踩过的坑。自己写提示词也一样，当 AI 在实际使用中出了问题，最好的修复方式就是把具体的错误场景直接写进去。

**3）给搜索行为设定量化阈值。** 很多人写提示词关于搜索就写一句「必要时搜索」，但 Anthropic 把搜索做成了一套决策流程：不搜索不变的事实（比如勾股定理）；必须搜索可能变化的状态（比如谁是某公司 CEO）；甚至规定好了「简单问题搜 1 次，中等问题搜 3 ~ 5 次，深度研究搜 5 ~ 10 次」。越量化的指令，AI 越容易遵循。

**4）输出格式影响用户感受。** 提示词里有一条规定：拒绝请求时不准用列表格式。因为列表给人冷冰冰的机械感觉，而拒绝时需要温度和同理心。如果你在做 AI 客服或 AI 伴侣类产品，格式不只是排版问题，它会影响用户的情绪体验。
